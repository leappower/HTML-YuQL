...
function shouldWriteTranslation(lang, existingValue, sourceChinese) {
  if (lang === 'zh' || lang === 'zh-CN') return !existingValue;
...
async function translateOneLang(lang, li, totalLangs, productSeries, translations, pendingKeys = null) {
  ...
  for (const { key, chineseText } of pendingEntries) {
    ...
    if (shouldWriteTranslation(lang, existingVal, chineseText)) {
...
/**
 * 处理中文源语言（zh / zh-CN）：直接写入原文，无需调用翻译 API
 */
...
async function translateProducts() {
  ...
  const targetLangs = SUPPORTED_LANGS.filter(l => l !== 'zh' && l !== 'zh-CN');
